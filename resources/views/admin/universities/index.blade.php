<div>
    @can('university-create')
        <div class="flex justify-between items-center mb-6">
            <x-breadcrumbs currentUrl="admin/universities" />
            <div class="flex justify-end mb-3">
                <x-button.secondary-button type="button" wire:navigate
                    href="{{ route('admin.universities', ['action' => 'create']) }}">
    ...
    <x-datatable.table>
        ...
        <x-slot name="tableBody">
            <x-datatable.body>
                ...
                <x-slot name="body">
                    @forelse($universities as $index => $record)
                        <tr
                                ...
                                <x-select.action-dropdown>
                                    @can('university-show')
                                        <li class="hover:bg-gray-100 dark:hover:bg-gray-600">
                                            <a href="{{ route('admin.universities', ['action' => 'show', 'id' => $record->id]) }}"
                                    ...
                                    @can('university-edit')
                                        <li class="hover:bg-gray-100 dark:hover:bg-gray-600">
                                            <a href="{{ route('admin.universities', ['action' => 'edit', 'id' => $record->id]) }}"
                                                wire:navigate class="flex items-center gap-2 px-4 py-2">
    ...
    <div class="p-4 bg-gray-100 dark:bg-gray-800 rounded-b-xl">
        @if ($universities)
            {{ $universities->links(data: ['scrollTo' => true]) }}
        @endif
    </div>
</div>
