<div>
    @can('ministry-create')
        <div class="flex justify-between items-center mb-6">
            <x-breadcrumbs currentUrl="admin/ministries" />
            <div class="flex justify-end mb-3">
                <x-button.secondary-button type="button" wire:navigate
                    href="{{ route('admin.ministries', ['action' => 'create']) }}">
    ...
    <x-datatable.table>
        ...
        <x-slot name="tableBody">
            <x-datatable.body>
                ...
                <x-slot name="body">
                    @forelse($ministries as $index => $record)
                        <tr
                                ...
                                <x-select.action-dropdown>
                                    @can('discipline-edit')
                                        <li class="hover:bg-gray-100 dark:hover:bg-gray-600">
                                            <a href="{{ route('admin.ministries', ['action' => 'edit', 'id' => $record->id]) }}"
    ...
    <div class="p-4 bg-gray-100 dark:bg-gray-800 rounded-b-xl">
        @if ($ministries)
            {{ $ministries->links(data: ['scrollTo' => true]) }}
        @endif
    </div>
</div>
